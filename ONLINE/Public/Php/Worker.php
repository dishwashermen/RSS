...
function NewUser($id, $data, $provisional = null, $hash = false) {
	...
	$query = 'INSERT INTO `users` (`id`, `prid`, `data1`' . (count($data) > 1 ? ', `data2`' : '') . ($hash ? ', `hash`' : '') . ', `UserAgent`, `Status`) VALUES (' . $EmptyId . ', :prId, :data1' . (count($data) > 1 ? ', :data2' : '') . ($hash ? ', :hash' : '') . ', :UserAgent, 1)';
	
	$array = array('prId' => $id, 'data1' => $data[0], 'UserAgent' => $_SERVER['HTTP_USER_AGENT']);
	
	if (count($data) > 1) $array = array_merge($array, array('data2' => $data[1]));

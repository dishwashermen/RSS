...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (preg_array_key_exists('/ProjectId|UserId|StateIndex/', $_POST) && is_numeric($_POST['ProjectId']) && is_numeric($_POST['UserId'])) {
		...
		foreach ($_POST as $key => $val) if (! Preg_match('/ProjectId|UserId|DirectRule|Autofill|StateIndex|HistoryState|Total|Difference|DiffContent|JournalIndex|JournalState|Limit|ViewContent/', $key, $matches)) {
		...
		if (! isset($_POST['JournalState']) || $_POST['JournalState'] == 'END') {
			...
			// Обновление квот при завершении -----------------------------------------------------------------------------------

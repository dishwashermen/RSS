	...
	LoginPage: function() {
		...
		switch (PROJECT.AuthType) {
			...
			case 'base':
				...
				let InputBox1 = new elem('div', {id: 'Login1', classname: 'input-box'});
				
				let InputBox2 = new elem('div', {id: 'Login2', classname: 'input-box'});

				InputBox1.append(new elem('input', {type: 'text', attr: 'name=1*required*list=List1', addevent: (PROJECT.AuthData == 2 ? 'blur' : '')}));
				
				InputBox1.append(new elem('label', {textcontent: BASEDATA.FieldName[0]}));
				
				let InputList1 = new elem('datalist', {id: 'List1'});
				
				let InputList2 = new elem('datalist', {id: 'List2'});
				
				InputBox1.append(InputList1);
				
				InputBox2.append(InputList2);
				
				if (PROJECT.AuthData == 2) {

					InputBox2.append(new elem('input', {type: 'text', attr: 'name=2*required*list=List2', addevent: 'blur'}));
				
					InputBox2.append(new elem('label', {textcontent: BASEDATA.FieldName[1]}));

				}
				
				for (let f in BASEDATA.FieldData) {
					
					let Opt1 = new elem('option', {textcontent: f});
					
					InputList1.append(Opt1);
					
					if (PROJECT.AuthData == 2) [].forEach.call(BASEDATA.FieldData[f], function(x) {
						
						let Opt2 = new elem('option', {textcontent: x});
					
						InputList2.append(Opt2);
						
					});
					
				}
				
				document.querySelector('#Password').insertAdjacentElement('beforeBegin', InputBox1);
				
				if (PROJECT.AuthData == 2) document.querySelector('#Password').insertAdjacentElement('beforeBegin', InputBox2);
	...
	switch (e.type) {
		...
		case 'blur':
			...
			if (el.name == 1) {
				...
				let InputList2 = document.querySelector('#Login2').appendChild(new elem('datalist', {id: 'List2'}));
				
				if (el.value.trim() != '' && BASEDATA.FieldData.hasOwnProperty(el.value.trim())) {

					[].forEach.call(BASEDATA.FieldData[el.value.trim()], function(x) {
				...
				if (Input1.value.trim() == '') {
					...
					for (let f in BASEDATA.FieldData) {
						...
						if (BASEDATA.FieldData[f].includes(el.value.trim())) {
					...
					if (Value1 !== false) Input1.value = Value1;
					...
					if (CANSEND) {
						...
						switch (PROJECT.AuthType) {
							...
							case 'base':
								...
								InputValue = document.querySelector('#Login1 input').value.trim();
							
								if (BASEDATA.FieldData.hasOwnProperty(InputValue)) AuthData = InputValue;
								
								else AuthError = true;
								
								if (PROJECT.AuthData == 2 && ! AuthError) {
									
									InputValue = document.querySelector('#Login2 input').value.trim();
									
									if (BASEDATA.FieldData[AuthData].includes(InputValue)) AuthData += ', ' + InputValue;

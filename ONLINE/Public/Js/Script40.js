document.addEventListener('DOMContentLoaded', function() {
	...
	if ($_GET('Hash') || $_GET('A') || $_GET('P')) {
		...
		ImportJS('md5, Dialog40, Valid40, Worker40');
		...
		else SendRequest('Public/Php/GetProject40.php', FD);
					...
					switch (RESP.Action) {
						...
						case 'Personal':
							...
							if (PROJECT.ViewMap) {
								...
								let ConcatData = ViewMap.concat(ViewData);
								
								[].forEach.call(ConcatData, function(x) {
...
PageManager = {
	...
	LoginPage: function() {
		...
		switch (REGTYPE.groups.type) {
			
			case 'user':
			
				switch (REGTYPE.groups.data) {
					...
					default:
						...
						document.querySelector('#Login label').textContent = REGTYPE.groups.data;
			...
			case 'file':
				...
				for (let i = 0; i < + REGTYPE.groups.data; i ++) {
					
					let InputBox = new elem('div', {id: 'Login' + (i + 1), classname: 'input-box'});
					
					let InputList = new elem('datalist', {id: 'List' + (i + 1)});
					
					[].forEach.call(FILEDATA[i].fieldData.flat(), function(y) {
					...
					InputBox.append(new elem('input', {type: 'text', attr: 'name=' + (i + 1) + '*required*list=List' + (i + 1), addevent: (REGTYPE.groups.data > 1 ? 'blur' : '')}));
					
					if (InputList.childNodes.length > 0) InputBox.append(InputList);
					
					InputBox.append(new elem('label', {textcontent: FILEDATA[i].fieldName}));
					
					document.querySelector('#Password').insertAdjacentElement('beforeBegin', InputBox);
		...
		case 'OFF':
			...
			if (Node.nodeName == 'TR') {
				...
				if (typeof(Node.dataset.protect) == 'undefined') {
					...
					[].forEach.call(Node.querySelectorAll('input, select'), function(i) { 
							...
							switch (RD.Operation) {
								...
								case 'CONTAINS': case 'NOT CONTAINS':
									...
									//for (let i = 0; i < QX.length; i ++) {
										...
										if ((RD.Operation == 'CONTAINS' && QX.includes(TX[0])) || (RD.Operation == 'NOT CONTAINS' && ! QX.includes(TX[0]))) {
					...
					for (let i = 1; i <= RowsQuantity; i ++) {
						...
						if ((RD.Action == 'DISPLAY' && MatchIndex > -1) || (RD.Action == 'NOT DISPLAY' && MatchIndex == -1)) {
							...
							if (ResponseData.Data[MatchIndex] != null && ResponseData.Data[MatchIndex] != '') {
								...
								Row.querySelector('input').setAttribute('data-auto-fill', '');
...
function builder() {
	...
	// Обработка строк для мобильных устройств ----------------------------------------------------------------------------------
	
	if (MOBILE && RowsContentData == null && InputDataData != null && InputDataData.length == 1) {
	
		MobileDisplay = true;
		
		RowsContentData = InputDataData[0].split('*');
	...
	for (let RowIndex = 0, id = 0; RowIndex < RowsQuantity; RowIndex ++) {
		...
		for (let ColIndex = 0; ColIndex < ColsQuantity; ColIndex ++) {
			...
			let TD = new elem('td', {classname: 'input-container', attr: 'data-n=' + (WorkCol ? WorkCol.Index : ColIndex) + (WorkCol && WorkCol.Attr ? '*' + WorkCol.Attr : '') + (WorkCol && WorkCol.Property ? '*' + WorkCol.Property : '')});
			...
			if (ColIndex > 0 || RowsQuantity == 1) {
				...
				if (WorkCol && WorkCol.Name && PromptEl == null) {

					PromptEl = WorkCol.Name.match(/^Друг(ое$|ие$|ая$|ой$)|\[(t|n)\]/);
				
					WorkCol.Name = WorkCol.Name.replace(/\[t\]|\[n\]/, '');
				...
				if (! MobileDisplay && (WorkInputType == 'radio' || WorkInputType == 'checkbox')) {
	...
	if (! HISTORY || STATEINDEX == '1' || STATEINDEX == PROJECT.Total || !! QDATA.BackDisable) document.querySelector('#Back').classList.add('hidden');
	
	else document.querySelector('#Back').classList.remove('hidden');
	
	// --------------------------------------------------------------------------------------------------------------------------
	
	// Отображение кнопки Далее
	
	if (STATEINDEX == PROJECT.Total) document.querySelector('#Submit').classList.add('hidden');
		
	else document.querySelector('#Submit').textContent = HasInput ? 'Сохранить ответ и перейти к следующему вопросу' : 'Следующий вопрос';
...
function listener(e) {
	...
	switch (e.type) {
		...
		case 'blur':
			...
			if (REGTYPE.groups.data > el.name) {
				
				let Index = FILEDATA[el.name - 1].fieldData.flat().indexOf(el.value);
				
				let NextIndex = + el.name + 1;
				
				document.querySelector('#List' + NextIndex).remove();
				
				let DomList = document.querySelector('#Login' + NextIndex).appendChild(new elem('datalist', {id: 'List' + NextIndex}));
				
				if (Index != -1 || el.value.trim() == '') {
					
					let OptList = Index != -1 ? FILEDATA[el.name].fieldData[Index] : FILEDATA[el.name].fieldData.flat();
					
					[].forEach.call(OptList, function(y) {
			...
			if (el.name > 1) {
				...
				let PrevIndex = + el.name - 1;
				
				let PrevEl = document.querySelector('#Login' + PrevIndex + ' input');
				
				if (PrevEl.value.trim() == '') {
					
					let Index = false;
					
					for (let i = 0; i < FILEDATA[PrevIndex].fieldData.length; i ++) if (FILEDATA[PrevIndex].fieldData[i].includes(el.value)) {
					...
					if (Index !== false) PrevEl.value = FILEDATA[PrevIndex - 1].fieldData[Index][0];
			...
			switch (el.id) {
								...
								case 'radio':
									...
									if (E && typeof(E[0]) != 'string') {
										...
										if (RadioValue != '') {
											...
											DATACOMPLETED[i][2] = E[0].parentNode.parentNode.querySelector('.row') != null ? E[0].parentNode.parentNode.querySelector('.row').textContent : E[0].parentNode.querySelector('label[for="' + E[0].id + '"]').textContent;
						...
						if (E) { // display error
							...
							if (CANSEND) {
								...
								if (! NODATA) for (let i in DATACOMPLETED) {
									...
									if (VIEW != null && VIEW.hasOwnProperty(i)) VIEWCONTENT[VIEW[i]] = DATACOMPLETED[i][0] == 'select' || DATACOMPLETED[i][0] == 'radio' ? '[' + DATACOMPLETED[i][1][0] + ']' + DATACOMPLETED[i][2] : DATACOMPLETED[i][1][0];
								...
								SendRequest('Public/Php/Write40.php', FD);
				...
				case 'LoginSubmit':
					...
					if (CANSEND) {
						...
						switch (REGTYPE.groups.type) {
							...
							case 'user':
								...
								switch (Input.type) {
									...
									case 'tel':
										...
										if (/_/.test(InputValue)) AuthError = true;
								
										else AuthData = InputValue;
							...
							case 'file':
								...
								for (let i = 0; i < + REGTYPE.groups.data; i ++) {
									...
									InputValue = document.querySelector('#Login' + (i + 1) + ' input').value.trim();
									
									LastADIndex = FILEDATA[i].fieldData.flat().indexOf(InputValue);
									...
									} else AuthData += (AuthData != '' ? ', ' : '') + InputValue;
						...
						if (REGTYPE.groups.mode == 'PROTECT' && ! AuthError) {
						...
						if (AuthError === false) {
							...
							if (FILEDATA != null && FILEDATA.length > + REGTYPE.groups.data) for (let i = + REGTYPE.groups.data; i < FILEDATA.length; i ++) AD[FILEDATA[i].fieldName] = FILEDATA[i].fieldData.flat()[LastADIndex];
							
							FD.append('AuthData', AuthData);
							
							if (REGTYPE.groups.mode == 'PROTECT') FD.append('AuthHash', AuthHash);
				...
				case 'Back':
					...
					if (CANSEND) {
						...
						SendRequest('Public/Php/Back40.php', FD);

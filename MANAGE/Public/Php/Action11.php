...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && count($_POST) >= 3 && isset($_POST['ProjectId']) && is_numeric($_POST['ProjectId'])) {
	...
	switch ($_POST['Action']) {
		...
		case 'UserRemove':
			...
			$DeleteUser = $DB -> prep('DELETE FROM `users` WHERE `users`.`id` = ' . $_POST['UserId']);
			
			$DeleteUserTable = $DBQ -> prep('DROP TABLE u' . $_POST['UserId']);
			
			echo json_encode(array('Action' => 'UserRemove', 'Id' => $_POST['UserId'], 'UserIndex' => $_POST['UserIndex']));
